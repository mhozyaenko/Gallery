...
const State = {
    filteredList: () => {
        if (initialState.isFirstRender) {
            ...
            result = result.filter(item => item.width >= initialState.minWidth && item.width <= initialState.maxWidth && item.height >= initialState.minHeight && item.height <= initialState.maxHeight);
